...
export default function proxy(request: NextRequest) {
  ...
  // Preview protection (si activée)
  const protectPreview = process.env.PREVIEW_PROTECT === 'true';
  if (protectPreview) {
    ...
    if (!isAsset) {
      ...
      const cookieToken = request.cookies.get('preview_token')?.value || '';
  ...
  response.headers.set(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline' https://vercel.live https://va.vercel-scripts.com https://*.sentry.io; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' data:; connect-src 'self' https: wss:; worker-src 'self' blob:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests;"
  );
