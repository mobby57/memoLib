...
async function createAdmin() {
  ...
  try {
    ...
    const admin = await p.user.upsert({
      where: { email: 'admin@iaposte.fr' },
      update: { password: hash },
      create: {
        email: 'admin@iaposte.fr',
        name: 'Admin Demo',
        password: hash,
        role: 'ADMIN',
        tenantId: tenant.id
    ...
    const avocat = await p.user.upsert({
      where: { email: 'avocat@iaposte.fr' },
      update: { password: hash },
      create: {
        email: 'avocat@iaposte.fr',
        name: 'Avocat Demo',
        password: hash,
        role: 'LAWYER',
        tenantId: tenant.id
    ...
    console.log('========================================');
    console.log('   Admin:  admin@iaposte.fr / admin123');
    console.log('   Avocat: avocat@iaposte.fr / admin123');
    console.log('========================================\n');
