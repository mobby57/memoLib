...
if POSTGRES_AVAILABLE and os.getenv('SUPABASE_DB_URL'):
    ...
    print("✅ PostgreSQL Supabase configuré")
else:
    # Fallback SQLite
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///data/app.db'
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    print("⚠️ Fallback SQLite activé")
...
if POSTGRES_AVAILABLE:
    try:
        ...
        print("✅ Middleware sécurité activé")
    except:
        print("⚠️ Sécurité en mode dégradé")
    ...
    <script>
        ...
        async function loadHistory() {
            ...
            document.getElementById('emailHistory').innerHTML = emails.map(e => {
                const statusIcon = e.status === 'sent' ? '✅' : '⚠️';
                const statusText = e.status === 'sent' ? 'Envoyé' : 'Stocké seulement';
