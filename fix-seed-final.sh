...
echo -e "${YELLOW}📝 Réécriture complète de $FILE en ESM pur...${NC}"

# Sauvegarde
cp "$FILE" "$FILE.bak.$(date +%s)"

# Écraser avec le contenu correct
cat > "$FILE" << 'SEED'
/**
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Seed exécuté avec succès. Vous pouvez relancer les tests.${NC}"
else
