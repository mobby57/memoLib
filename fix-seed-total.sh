...
echo -e "${YELLOW}📝 Réécriture complète du seed avec TOUS les champs du modèle Plan...${NC}"

# Sauvegarde
cp prisma/seed-e2e.ts prisma/seed-e2e.ts.bak.$(date +%s)

# Écraser avec le contenu complet
...
async function main() {
  // 1. Plan par défaut (TOUS les champs du modèle)
  ...
  // 7. Email (optionnel mais utile)
  await prisma.email.upsert({
    where: { id: 'email1' },
    update: {},
    create: {
      id: 'email1',
      subject: 'URGENT - Demande de regroupement familial',
...
echo -e "${GREEN}✅ Fichier réécrit avec tous les champs.${NC}"
