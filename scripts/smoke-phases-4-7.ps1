...
if (-not $selectedPort) {
    throw 'Aucun port libre trouvé entre 8095 et 8110.'
}
...
$env:ConnectionStrings__Default = 'Data Source=memolib.smoke.db'
...
try {
    ...
    if (-not $token) {
        throw 'Échec login: token JWT absent.'
    }

    $headers = @{ Authorization = "Bearer $token" }

    $ext1 = "smoke-mail-1-$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())"
    $ext2 = "smoke-mail-2-$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())"
