-- ============================================================
-- seed-test-fixed.sql
-- Idempotent : les données existantes sont ignorées
-- ============================================================
...
-- 3. Utilisateurs (admin et avocat)
-- Admin
INSERT INTO "User" (id, email, name, password, role, "tenantId", status, language, timezone, "createdAt", "updatedAt")
SELECT
  gen_random_uuid(), 'admin@test.com', 'Admin Test', '$2a$12$...', 'ADMIN', 
  (SELECT id FROM "Tenant" WHERE subdomain = 'test-cabinet'), 'active', 'fr', 'Europe/Paris', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM "User" WHERE email = 'admin@test.com');

-- Avocat
INSERT INTO "User" (id, email, name, password, role, "tenantId", status, language, timezone, "createdAt", "updatedAt")
SELECT
  gen_random_uuid(), 'avocat@test.com', 'Avocat Test', '$2a$12$1C26unvsJp5LdZp6XhA5L.lCuI2UJYLwvYuzYvBrBEJEHbBgqV/E.', 'USER', 
  (SELECT id FROM "Tenant" WHERE subdomain = 'test-cabinet'), 'active', 'fr', 'Europe/Paris', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM "User" WHERE email = 'avocat@test.com');
...
-- 8. Ligne de facture (associée à la facture créée)
...
WHERE EXISTS (
  SELECT 1 FROM "Facture" 
  WHERE "tenantId" = (SELECT id FROM "Tenant" WHERE subdomain = 'test-cabinet')
);
