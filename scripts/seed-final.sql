...
-- 2. Tenant (si pas de contrainte unique sur subdomain, on utilise WHERE)
...
-- 4. Client (ON CONFLICT sur tenantId + email)
...
-- 5. Dossier (ON CONFLICT sur tenantId + numero)
...
-- 6. Délai légal (ON CONFLICT sur tenantId + dossierId + label)
...
-- 7. Facture (ON CONFLICT sur tenantId + numero)
