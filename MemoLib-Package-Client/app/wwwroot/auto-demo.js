    ...
    async function runAutoDemo() {
        ...
        const steps = [
            ...
            { name: '📧 Ingestion emails', action: async () => {
                ...
                await wait(1000);
                const emails = [
                    { from: 'client.divorce@example.com', subject: 'Demande divorce urgent', body: 'Je souhaite entamer une procédure de divorce. Situation urgente.', ext: `DEMO-DIV-${Date.now()}-1` },
                    { from: 'client.travail@example.com', subject: 'Licenciement abusif', body: 'Mon employeur m\'a licencié sans motif valable.', ext: `DEMO-TRA-${Date.now()}-2` },
                    { from: 'client.immobilier@example.com', subject: 'Litige propriétaire', body: 'Conflit avec mon propriétaire sur les charges.', ext: `DEMO-IMM-${Date.now()}-3` }
                ];
