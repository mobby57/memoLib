...
def test_path_traversal(path: str) -> dict:
    ...
    return {"original": path, "normalized": normalized, "blocked": dangerous}
...
def run_all_tests():
    ...
    # Note: Détecter un mot de passe FAIBLE = protection RÉUSSIE (on le rejette)
    ...
    for password in BRUTE_FORCE_PASSWORDS[:8]:
        ...
        # Un mot de passe faible détecté = protection fonctionnelle
        is_weak_detected = result["strength"] != "FORT"
        status = "BLOCKED" if is_weak_detected else "VULNERABLE"
        if is_weak_detected:
            results["auth"]["weak"] += 1
        log_attack(f"Mot de passe faible '{result['password']}' détecté", status)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type",
        choices=["sql", "xss", "auth", "path", "nosql", "cmd", "all"],
        default="all",
