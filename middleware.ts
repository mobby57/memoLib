...
export function middleware(request: NextRequest) {
  try {
    ...
    if (/\/demo\/(?:l%C3%A9gal-proof|légal-proof)$/i.test(pathname)) {
    ...
    // Safety: next-intl middleware can return undefined for ignored requests.
    ...
    // Never break request flow if middleware throws at runtime on edge.
    return NextResponse.next();
  }
}
