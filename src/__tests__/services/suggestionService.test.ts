...
describe('Suggestion Service', () => {
  ...
  describe('Stale Dossier Detection', () => {
    ...
    const getDaysWithoutUpdate = (lastUpdate: Date, now: Date = createAnchorDate()): number => {
    ...
    it('devrait détecter un dossier obsolète', () => {
      ...
      expect(isStale(oldDate, 14, now)).toBe(true);
    });
    ...
    it('devrait ne pas détecter un dossier récent', () => {
      ...
      expect(isStale(recentDate, 14, now)).toBe(false);
    });
    ...
    it('devrait calculer les jours sans mise à jour', () => {
      ...
      expect(getDaysWithoutUpdate(tenDaysAgo, now)).toBe(10);
    });
  });
