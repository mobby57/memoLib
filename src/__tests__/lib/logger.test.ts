...
describe('Logger - Systeme de logging professionnel', () => {
  ...
  describe('Niveaux de log', () => {
    test('debug() log uniquement en developpement', () => {
      ...
      // debug() peut ou non logger selon la config
      const logs = logger.getBufferedLogs();
      const initialCount = logs.length;

      // debug() ne devrait pas générer d'erreur
      expect(() => logger.debug('Test debug', { data: 'test' })).not.toThrow();

      // En environnement test, debug peut être bufferisé ou non selon la config
  ...
  describe('Logs metier - Actions dossiers', () => {
    test('logDossierAction() trace action juridique', () => {
      ...
      logDossierAction('CREATE_DOSSIER', 'user-123', 'tenant-456', 'dossier-789', {
        clientId: 'client-001',
        typeDossier: 'OQTF',
    ...
    test('logIAUsage() trace utilisation IA', () => {
      logIAUsage('ANALYSIS', 'user-456', 'tenant-789', 'dossier-123', {
        confidence: 0.95,
        modelUsed: 'llama3.2',
        inputType: 'OQTF',
    ...
    test('logRGPDAction() trace actions RGPD', () => {
      logRGPDAction('EXPORT_DATA', 'user-789', 'tenant-123', 'client-456', {
  ...
  describe('Audit juridique', () => {
    test('audit() trace actions avec RGPD compliance', () => {
      ...
      logger.audit('Modification dossier OQTF', 'user-audit', 'tenant-audit', {
        dossierId: 'DOSSIER-999',
        changes: ['deadline', 'status'],
      });
