...
class Database:
    ...
    def add_contact(self, name, email, organization='', category='custom'):
        """Ajoute un nouveau contact"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO contacts (name, email, organization, category)
                VALUES (?, ?, ?, ?)
    ...
    def get_all_contacts(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            result = {'custom': [], 'frequent': [], 'institutions': {}}
            for contact in contacts:
                category = contact.get('category', 'custom')
                if category in ['custom', 'frequent']:
                    result[category].append(contact)
                else:
                    if category not in result['institutions']:
                        result['institutions'][category] = []
                    result['institutions'][category].append(contact)
