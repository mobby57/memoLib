...
export default function ProceduresTab({ procedures, onRefresh }: ProceduresTabProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header avec bouton Nouvelle Procédure */}
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Procédures Juridiques</h3>
          <p className="text-sm text-gray-600 mt-1">{procedures.length} procédure(s) au total</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            <div key={proc.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-6 border-b border-gray-200">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[proc.status as keyof typeof statusColors]}`}>
                        {proc.status === 'active' ? 'En cours' : proc.status === 'pending' ? 'En attente' : 'Terminé'}
                      </span>
                      {proc.reference && (
                        <span className="text-sm text-gray-500">Réf: {proc.reference}</span>
                      )}
                    </div>
                  ...
                  {daysRemaining !== null && (
                    <div className={`px-4 py-2 rounded-lg text-center ${
                      ...
                      <div className="text-xs">
                        {daysRemaining < 0 ? 'jours dépassés' : 'jours restants'}
                      </div>
                    </div>
                  )}
                </div>
                ...
                {Object.keys(metadata).length > 0 && (
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs font-semibold text-gray-700 mb-2">Détails procédure</p>
              ...
              <div className="p-6 bg-gray-50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-sm font-medium text-gray-700">{progress}% complété</span>
                </div>
              ...
              <div className="p-4 bg-white border-t border-gray-200 flex items-center justify-between">
                <div className="flex items-center space-x-4 text-sm text-gray-600">
                  ...
                  <span className="flex items-center">
                    ...
                    {proc.echeances.length} échéances
                  </span>
        ...
        {procedures.length === 0 && (
          <div className="text-center py-12 text-gray-500 bg-white rounded-lg border border-gray-200">
            ...
            <p className="text-lg font-medium">Aucune procédure</p>
            <p className="text-sm mt-2">Créez une nouvelle procédure pour commencer</p>
