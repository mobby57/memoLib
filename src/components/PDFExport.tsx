...
function generateFacturePDF(doc: any, facture: any) {
  ...
  const tableData = [
    ...
    [facture.description || 'Prestation juridique', `${facture.montant.toFixed(2)} e`]
  ];
  ...
  doc.text(`${facture.montant.toFixed(2)} e`, pageWidth - 30, finalY, { align: 'right' });
  
  doc.text('TVA (20%):', pageWidth - 80, finalY + 7);
  doc.text(`${(facture.montant * 0.20).toFixed(2)} e`, pageWidth - 30, finalY + 7, { align: 'right' });
  
  doc.setFontSize(12);
  doc.setFont(undefined, 'bold');
  doc.text('TOTAL TTC:', pageWidth - 80, finalY + 17);
  doc.text(`${montantTTC.toFixed(2)} e`, pageWidth - 30, finalY + 17, { align: 'right' });
