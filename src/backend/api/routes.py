...
from ..security.input_validator import InputValidator, validate_password_strength
from ..security.middleware import intrusion_detection, rate_limit, secure_endpoint
...
@rate_limit("auth")  # 5 tentatives par minute
@intrusion_detection
def api_login():
    """
    Login API - Retourne JWT token avec authentification sécurisée
    ...
    try:
        ...
        validation = InputValidator.validate_dict(data or {})
        if not validation["valid"]:
            logger.warning(
                f"Tentative d'injection détectée lors du login: {validation['attacks']}"
            )
            return jsonify({"success": False, "error": "Entrée invalide"}), 400
        ...
        try:
            ...
            try:
                ...
                if not user:
                    ...
                    time.sleep(0.5)
                    logger.warning(
                        f"Tentative de login échouée pour email inexistant: {email}"
                    )
                    return (
                ...
                access_token = create_access_token(
                    ...
                    expires_delta=timedelta(hours=8),  # Durée réduite pour la sécurité
                    additional_claims={
                        "email": user.email,
            ...
            # Fallback si la base de données n'est pas configurée
            logger.error("Base de données non configurée pour l'authentification")
            return (
                jsonify(
                    {
                        "success": False,
        ...
        logger.error(f"Erreur lors du login: {str(e)}")
...
def api_get_cases():
    ...
    try:
        ...
        return jsonify({"success": True, "data": cases, "total": len(cases)}), 200
...
def api_create_case():
    ...
    try:
        ...
        return jsonify({"success": True, "data": new_case}), 201
...
def api_ai_analyze():
    ...
    try:
        ...
        analysis = {
            ...
            "factors_positive": [kw for kw in positive_keywords if kw in text_lower],
            "factors_negative": [kw for kw in negative_keywords if kw in text_lower],
            "recommendations": (
                [
            ...
            "urgency": (
                "ELEVEE"
                if "oqtf" in text_lower or "expulsion" in text_lower
                else "NORMALE"
            ),
        }
...
def api_get_invoices():
    ...
    try:
        ...
        return jsonify({"success": True, "data": invoices, "total": len(invoices)}), 200
...
def api_create_invoice():
    ...
    try:
        ...
        if not data.get("client") or not data.get("amount_ht"):
            return (
                jsonify(
                    {"success": False, "error": "Client and amount_ht are required"}
                ),
                400,
            )
        ...
        return jsonify({"success": True, "data": new_invoice}), 201
