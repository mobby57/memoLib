...
export function initializeWebSocket(httpServer: HTTPServer): SocketIOServer {
  ...
  io.on('connection', (socket) => {
    ...
    console.log(`[WebSocket] Client connected: ${socket.id} (User: ${userId}, Tenant: ${tenantId})`)
    ...
    socket.on('join-tenant', (joinTenantId: string) => {
      if (tenantId !== joinTenantId) {
        console.warn(`[WebSocket] User ${userId} attempted to join different tenant ${joinTenantId}`)
        return
      }
    ...
    socket.on('join-user', (joinUserId: string) => {
      if (userId !== joinUserId) {
        console.warn(`[WebSocket] User ${userId} attempted to join different user room ${joinUserId}`)
        return
      }
