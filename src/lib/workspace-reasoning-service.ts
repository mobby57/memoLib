...
export class WorkspaceReasoningService {
    ...
    switch (targetState) {
      ...
      case 'OBLIGATIONS_DEDUCED':
        const confirmedContexts =
          workspace.contextHypotheses?.filter(c => c.certaintyLevel === 'CONFIRMED') || [];
  ...
  static generateExecutiveSummary(workspace: WorkspaceReasoning): string {
    ...
    const contexts =
      workspace.contextHypotheses?.filter((c: any) => c.certaintyLevel === 'CONFIRMED').length || 0;
    ...
    summary += `Qualite: ${((workspace.reasoningQuality ?? 0) * 100).toFixed(0)}%\n\n`;
