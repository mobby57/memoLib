/**
 * RBAC — logique pure isomorphe (serveur ET client).
 *
 * Ce module ne dépend d'AUCUN runtime serveur (pas de `next/server`), il peut
 * donc être importé côté client (hooks React) comme côté serveur (routes API).
 *
 * IMPORTANT — sécurité : ce module sert UNIQUEMENT à décider de l'affichage
 * côté UI (masquer/afficher un bouton). Il n'est JAMAIS une frontière de
 * sécurité. Toute action doit être re-vérifiée côté serveur via
 * `requireApiPermission` (voir rbac.ts).
 */
