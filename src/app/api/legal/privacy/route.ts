...
export async function GET(req: NextRequest) {
    try {
        ...
        // Generate Privacy Policy for jurisdiction
        const privacy = legalDocs.generatePrivacyPolicy(jurisdiction);
...
<head>
  ...
  <title>Privacy Policy - MemoLib</title>
  <style>
...
<body>
  <div class="meta">
    ...
    <p><strong>Effective Date:</strong> ${privacy.effectiveDate.toLocaleDateString()}</p>
    <p><strong>Last Updated:</strong> ${privacy.lastUpdated.toLocaleDateString()}</p>
    <p><strong>Jurisdiction:</strong> ${jurisdiction}</p>
  </div>
  ${privacy.content.replace(/^# /gm, '<h1>').replace(/\n\n/g, '</p><p>').replace(/^/g, '<p>').replace(/$/, '</p>')}
</body>
</html>`
        });
    } catch (error: any) {
