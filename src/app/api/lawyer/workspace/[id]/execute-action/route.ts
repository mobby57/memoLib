...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (action.executed) {
      return NextResponse.json({ error: 'Action deja executee' }, { status: 400 });
    }
    ...
    logger.error('Erreur exécution action', error instanceof Error ? error : undefined, {
      route: '/api/lawyer/workspace/[id]/execute-action',
    });
