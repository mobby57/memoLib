...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (workspace.locked) {
      return NextResponse.json({ error: 'Workspace verrouille' }, { status: 400 });
    }
    ...
    if (!missingElementId || !resolution) {
      return NextResponse.json({ error: 'missingElementId et resolution requis' }, { status: 400 });
    }
    ...
    if (!missingElement) {
      return NextResponse.json({ error: 'Element manquant non trouve' }, { status: 404 });
    }
    ...
    logger.error('Erreur resolution element manquant', error instanceof Error ? error : undefined, {
      route: '/api/lawyer/workspace/[id]/resolve-missing',
    });
