...
import {
  ...
  CheckCircle, BarChart3, FileText, Bell, Lock,
...
const FEATURES = [
  { icon: Mail, title: 'Inbox intelligent', description: "Vos emails clients triés, classés et analysés automatiquement. Extraction des coordonnées, détection d'urgence.", color: 'blue' },
  { icon: Brain, title: 'IA spécialisée CESEDA', description: "Calcul automatique des délais légaux, suggestions de procédure, analyse du dossier en temps réel.", color: 'purple' },
  { icon: Scale, title: 'Workflow juridique', description: "De la réception de l'email à la clôture du dossier. Statuts, priorités, échéances, tout est tracé.", color: 'green' },
  { icon: Bell, title: 'Alertes échéances', description: "Plus jamais un délai raté. Alertes automatiques pour OQTF, recours, audiences, rendez-vous préfecture.", color: 'red' },
  { icon: FileText, title: 'Documents & preuves', description: "Génération automatique de documents, horodatage légal, signatures électroniques eIDAS.", color: 'amber' },
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <nav className="fixed top-0 w-full bg-white/80 dark:bg-gray-950/80 backdrop-blur-md z-50 border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/auth/login" className="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">Connexion</Link>
            <Link href="/auth/register?plan=PILOT" className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">Essai gratuit</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-6xl font-bold text-gray-900 dark:text-white mb-6 leading-tight">
            Email reçu. Dossier créé.<br /><span className="text-blue-600">Délai calculé.</span>
          </h1>
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-10 max-w-3xl mx-auto leading-relaxed">
            MemoLib transforme vos emails clients en dossiers structurés, calcule vos délais OQTF automatiquement, et vous alerte avant chaque échéance. Zéro deadline oubliée.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
            <Link href="/auth/register?plan=PILOT" className="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-8 rounded-xl transition-colors shadow-lg shadow-blue-600/25">
              Essai gratuit 14 jours <ArrowRight className="w-5 h-5" />
            </Link>
            <Link href="/demo/auto-login" className="inline-flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-900 dark:text-white font-semibold py-4 px-8 rounded-xl transition-colors">
          ...
          <div className="flex items-center justify-center gap-8 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span className="flex items-center gap-1.5"><Shield className="w-4 h-4 text-green-500" />RGPD natif</span>
            <span className="flex items-center gap-1.5"><Lock className="w-4 h-4 text-green-500" />Chiffrement AES-256</span>
          </div>
        </div>
      </section>
      ...
      <section id="tarifs" className="py-20 px-6 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl font-bold text-center text-gray-900 dark:text-white mb-4">Tarifs simples, sans surprise</h2>
          <p className="text-center text-gray-600 dark:text-gray-300 mb-12">Commencez gratuitement. Évoluez quand vous êtes prêt.</p>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {PRICING.map((plan, i) => (
              <div key={i} className={`rounded-2xl p-6 ${plan.highlighted ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/25 scale-105' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700'}`}>
                ...
                <Link href={plan.href} className={`block text-center py-3 px-4 rounded-xl font-semibold transition-colors text-sm ${plan.highlighted ? 'bg-white text-blue-600 hover:bg-blue-50' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                  {plan.cta}
                </Link>
              </div>
            ))}
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Vos données sont sacrées</h2>
          <p className="text-gray-600 dark:text-gray-300 mb-12 max-w-2xl mx-auto">Secret professionnel oblige. MemoLib est conçu pour la confidentialité absolue.</p>
