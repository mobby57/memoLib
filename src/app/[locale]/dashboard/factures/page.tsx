    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">{stats.total.toFixed(2)}e</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <p className="text-sm text-gray-500">Payees</p>
          <p className="text-2xl font-bold text-green-600">{stats.payees.toFixed(2)}e</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <p className="text-sm text-gray-500">En attente</p>
          <p className="text-2xl font-bold text-blue-600">{stats.enAttente.toFixed(2)}e</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <p className="text-sm text-gray-500">En retard</p>
          <p className="text-2xl font-bold text-red-600">{stats.enRetard.toFixed(2)}e</p>
        </div>
      </div>
              ...
              {filteredFactures.map((facture) => (
                <tr key={facture.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-medium">
                    {facture.montantTTC.toFixed(2)}e
                  </td>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex justify-between mb-1">
              <span>Total HT:</span>
              <span>{total.toFixed(2)}e</span>
            </div>
            <div className="flex justify-between mb-1">
              <span>TVA (20%):</span>
              <span>{tva.toFixed(2)}e</span>
            </div>
            <div className="flex justify-between font-bold text-lg">
              <span>Total TTC:</span>
              <span>{ttc.toFixed(2)}e</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
              ...
              <tbody>
                {facture.lignes.map((ligne, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="py-2 text-right">{ligne.prixUnitaire.toFixed(2)}e</td>
                    <td className="py-2 text-right">{ligne.montantHT.toFixed(2)}e</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between mb-1">
              <span>Total HT:</span>
              <span>{facture.montantHT.toFixed(2)}e</span>
            </div>
            <div className="flex justify-between mb-1">
              <span>TVA ({facture.montantTVA > 0 ? '20' : '0'}%):</span>
              <span>{facture.montantTVA.toFixed(2)}e</span>
            </div>
            <div className="flex justify-between font-bold text-lg">
              <span>Total TTC:</span>
              <span>{facture.montantTTC.toFixed(2)}e</span>
            </div>
          </div>
