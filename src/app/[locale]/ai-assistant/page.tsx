...
export default function AIAssistantPage() {
  ...
  const handleGenerateDocument = async () => {
    setLoading(true)
    const doc = await runUiAsyncAction({
      action: () => generateDocument('contrat', {
        parties: 'ABC et XYZ',
      ...
      showToast,
      successMessage: 'Document genere avec succes',
      fallbackErrorMessage: 'Erreur lors de la generation',
      context: { feature: 'ai-assistant', action: 'generate-document' },
    })
  ...
  const handleAnalyzeRisk = async () => {
    setLoading(true)
    const analysis = await runUiAsyncAction({
      action: () => analyzeRisk(mockDossier),
      showToast,
      successMessage: 'Analyse terminee',
      fallbackErrorMessage: "Erreur lors de l'analyse",
      context: { feature: 'ai-assistant', action: 'analyze-risk' },
    })
  ...
  const handleGetSuggestions = async () => {
    setLoading(true)
    const suggestions = await runUiAsyncAction({
      action: () => getSuggestions(mockDossier),
      showToast,
      successMessage: 'Suggestions generees',
      fallbackErrorMessage: 'Erreur lors de la generation des suggestions',
      context: { feature: 'ai-assistant', action: 'get-suggestions' },
    })
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
            {stats.estimatedCost.toFixed(2)} EUR
          </p>
        </Card>
        <Card className="p-4">
